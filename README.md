...
### Analysis part
The `analysis` element contains all the information required to run the workflow with the analysis. There is one required element - `command`, and two optional elements - `script` and `inputs`. The `command` could be "python" or "bash", and than the `script` will be needed, but it can be just "bet" and than the script is not required. The `inputs` part contains all the input needed to complete the command required to run the analysis. Each element of the `inputs` entry can have `type`, `argstr` (if a flag is needed) and `value`. If `type` is `File`, the file is assumed to be in the data directory. if `script` is provided, the file is expected to be in the scripts directory. An example can look like this:
